...
void testMapSerialization(const char* maptype, const char* fname)
{
    ...
    showtime("serialize", [&]() {
#if !USE_CEREAL
        gtl::BinaryOutputArchive ar_out(fname);
...
#if !USE_CEREAL
        gtl::BinaryInputArchive ar_in(fname);
...
int main()
{
    ...
    testMapSerialization<gtl::flat_hash_map<unsigned int, int>>("flat_hash_map", "dump1.bin");
    testMapSerialization<gtl::parallel_flat_hash_map<unsigned int, int>>("parallel_flat_hash_map", "dump1.bin");
#endif
