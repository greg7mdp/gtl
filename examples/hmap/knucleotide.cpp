...
struct Key
{
    ...
    bool operator==(const Key& in) const { return _data == in._data; }
...
using HashTable = gtl::flat_hash_map<K, unsigned, typename K::Hash>;
...
HashTable<size> CalculateInThreads(const Cfg::Data& input)
{
    ...
    auto invoke = [&](unsigned begin) { Calculate<size>(input, begin, hash_tables[begin]); };
