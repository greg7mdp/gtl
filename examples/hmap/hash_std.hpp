...
struct Person {
    bool operator==(const Person& o) const { return _first == o._first && _last == o._last && _age == o._age; }
...
template<>
struct hash<Person> {
    std::size_t operator()(Person const& p) const { return gtl::HashState().combine(0, p._first, p._last, p._age); }
};
}
