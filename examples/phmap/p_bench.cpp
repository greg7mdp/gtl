...
template<int n>
using pmap = gtl::parallel_flat_hash_map<uint64_t,
                                         uint64_t,
                                         std::hash<uint64_t>,
...
template<int n>
using pmap_nullmutex = gtl::parallel_flat_hash_map<uint64_t,
                                                   uint64_t,
                                                   std::hash<uint64_t>,
                                                   std::equal_to<uint64_t>,
                                                   std::allocator<std::pair<const uint64_t, uint64_t>>,
...
void renumber(const std::vector<uint64_t>& vertex_ids, std::vector<std::array<uint64_t, 4>> elements, int num_threads) {
    bool supports_parallel_insertion = !std::is_same<Map, std::unordered_map<uint64_t, uint64_t>>::value;
    ...
    pool.parallel_for(elements.size(), [&](uint64_t i) {
        ...
        elem       = { new_ids_nc.at(elem[0]), new_ids_nc.at(elem[1]), new_ids_nc.at(elem[2]), new_ids_nc.at(elem[3]) };
    });
    stopwatch.stop();
    std::cout << stopwatch.elapsed() * 1000 << "ms" << '\n';
}
...
int main() {
    ...
    for (uint64_t i = 0; i < nelements; i++) {
        elements[i] = { vertex_ids[elem_id_dist(gen)],
