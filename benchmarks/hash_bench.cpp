...
struct PMHash
{
    ...
    static void init() { polymur_init_params_from_seed(&p, 0xfedbca9876543210ULL); }

    static inline PolymurHashParams p;
};
...
int main()
{
    ...
    // using Map = gtl::flat_hash_map<std::string, uint32_t, PMHash>; // try using polymur_hash
    using Map = gtl::flat_hash_map<std::string, uint32_t>;
    Map map;
