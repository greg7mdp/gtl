...
TEST(vector, composition) { gtl::vector<gtl::vector<double>> matrix(100, gtl::vector<double>(100)); }
...
TEST(vector, move_iterator) {
    ...
    auto             cp1 = base;
    gtl::vector<int> fbvi1(std::make_move_iterator(cp1.begin()), std::make_move_iterator(cp1.end()));
    ...
    fbvi3.insert(fbvi3.end(), std::make_move_iterator(cp3.begin()), std::make_move_iterator(cp3.end()));
...
TEST(vector, stealing_constructor) {
    ...
    int* x;
    if constexpr (std::is_same_v<Alloc, std::allocator<vec_t::value_type>>) // always the case here, but left as an
