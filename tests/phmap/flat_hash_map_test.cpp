...
#include "unordered_map_constructor_test.hpp"
#include "unordered_map_lookup_test.hpp"
#include "unordered_map_members_test.hpp"
#include "unordered_map_modifiers_test.hpp"
...
template<class K,
         class V,
         class H     = gtl::priv::hash_default_hash<K>,
...
using MapTypes = ::testing::Types<Map<int, int>,
                                  Map<std::string, int>,
                                  Map<Enum, std::string>,
                                  Map<EnumClass, int>,
                                  Map<int, NonStandardLayout>,
...
TEST(THIS_TEST_NAME, LazyKeyPattern)
{
    ...
    // for parallel maps, the reserve(3) above is not sufficient to guarantee that a submap will not
    // resize and therefore rehash
    EXPECT_EQ(hashes, 3);
#endif
