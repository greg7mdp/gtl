...
TEST(Batch, DropDeletes)
{
    ...
    for (size_t i = 0; i < kCapacity + 1 + kGroupWidth; ++i) {
        ...
        if (IsFull(expected))
            expected = kDeleted;
        EXPECT_EQ(ctrl[i], expected) << i << " " << int{ pattern[i % pattern.size()] };
    }
}
...
TEST(Group, CountLeadingEmptyOrDeleted)
{
    ...
    for (ctrl_t empty : empty_examples) {
        ...
        for (ctrl_t full : full_examples) {
            ...
            EXPECT_EQ(Group::kWidth / 2, (int)Group{ f.data() }.CountLeadingEmptyOrDeleted());
        }
    }
}
...
struct IntPolicy
{
    ...
    static void transfer(void*, int64_t* new_slot, int64_t* old_slot) { *new_slot = *old_slot; }
...
public:
    ...
    static std::pair<std::string, std::string>& element(slot_type* slot) { return slot->pair; }
...
TEST(Table, EmptyFunctorOptimization)
{
    ...
    EXPECT_EQ(sizeof(MockTable),
              sizeof(raw_hash_set<StringPolicy,
                                  StatelessHash,
...
TEST(Table, NoThrowMoveAssign)
{
    ...
    ASSERT_TRUE(std::allocator_traits<std::allocator<int>>::is_always_equal::value);
...
TEST(Table, HeterogeneousLookupOverloads)
{
    ...
    using TransparentTable = raw_hash_set<StringPolicy,
