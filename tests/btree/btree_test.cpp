...
namespace {
    ...
    using ::gtl::test_internal::CopyableMovableInstance;
    using ::gtl::test_internal::InstanceTracker;
    using ::gtl::test_internal::MovableOnlyInstance;
...
public:
    ...
    IterType iter_check(IterType tree_iter, CheckerIterType checker_iter) const {
        if (tree_iter == tree_.end()) {
            GTL_INTERNAL_CHECK(checker_iter == checker_.end(),
    ...
    IterType riter_check(IterType tree_iter, CheckerIterType checker_iter) const {
        if (tree_iter == tree_.rend()) {
            GTL_INTERNAL_CHECK(checker_iter == checker_.rend(),
...
namespace {
    ...
    void MultiMapTest() {
        EXPECT_EQ(sizeof(gtl::btree_multimap<K, K>),
                  2 * sizeof(void *) +
                  sizeof(typename gtl::btree_multimap<K, K>::size_type));
        using BtreeMMap = gtl::btree_multimap<K, K>;
        using CountingBtreeMMap =
            gtl::btree_multimap<K, K, std::less<K>,
                                  PropagatingCountingAlloc<std::pair<const K, K>>>;
    ...
    TEST(Btree, NoHeterogeneousLookupWithoutAlias) {
        ...
        using StringMultiSet =
            gtl::btree_multiset<std::string, NonTransparentCompare>;
        StringMultiSet ms;
        ms.insert("hello");
        ms.insert("world");
        ms.insert("world");
    ...
    TEST(Btree, Comparison) {
        ...
        gtl::btree_set<int64_t> my_set_copy(my_set);
        ...
        gtl::btree_map<std::string, int64_t> my_map;
        for (int i = 0; i < kSetSize; ++i) {
            my_map[std::string(i, 'a')] = i;
        }
        gtl::btree_map<std::string, int64_t> my_map_copy(my_map);
    ...
    TEST(Btree, RangeCtorSanity) {
        ...
        gtl::btree_multiset<int> tmset(ivec.begin(), ivec.end());
        gtl::btree_multimap<int, int> tmmap(imap.begin(), imap.end());
        gtl::btree_set<int> tset(ivec.begin(), ivec.end());
        gtl::btree_map<int, int> tmap(imap.begin(), imap.end());
    ...
    TEST(Btree, InitializerListConstructor) {
        ...
        gtl::btree_multiset<int> mset({1, 1, 4});
        EXPECT_EQ(mset.count(1), 2);
        EXPECT_EQ(mset.count(4), 1);

        gtl::btree_map<int, int> map({{1, 5}, {2, 10}});
        EXPECT_EQ(map[1], 5);
        EXPECT_EQ(map[2], 10);

        gtl::btree_multimap<int, int> mmap({{1, 5}, {1, 10}});
    ...
    TEST(Btree, InitializerListInsert) {
        ...
        gtl::btree_multimap<int, int> mmap;
    ...
    TEST(Btree, RValueInsert) {
        ...
        gtl::btree_multiset<MovableOnlyInstance> mset;
        ...
        gtl::btree_map<int, MovableOnlyInstance> map;
        ...
        gtl::btree_multimap<int, MovableOnlyInstance> mmap;
...
namespace {
    ...
    TEST(Btree, MovesComparisonsCopiesSwapsTracking) {
        ...
        if constexpr (sizeof(void *) == 8) {
            EXPECT_EQ(BtreeNodePeer::GetNumValuesPerNode<gtl::btree_set<int32_t>>(),
    ...
    TEST(Btree, MapAt) {
        gtl::btree_map<int, int> map = {{1, 2}, {2, 4}};
        EXPECT_EQ(map.at(1), 2);
        EXPECT_EQ(map.at(2), 4);
        map.at(2) = 8;
        const gtl::btree_map<int, int> &const_map = map;
    ...
    TEST(Btree, StrSplitCompatible) {
        const gtl::btree_set<std::string> split_set = gtl::StrSplit("a,b,c", ',');
        const gtl::btree_set<std::string> expected_set = {"a", "b", "c"};
    ...
    TEST(Btree, ValueComp) {
        ...
        gtl::btree_map<std::string, int> m2;
    ...
    TEST(Btree, SwissTableHashable) {
        ...
        using set = gtl::btree_set<int>;
        EXPECT_TRUE(gtl::VerifyTypeImplementsPhmapHashCorrectly({
                    set{},
                        set{1},
                            set{2},
                                set{1, 2},
        ...
        using mset = gtl::btree_multiset<int>;
        EXPECT_TRUE(gtl::VerifyTypeImplementsPhmapHashCorrectly({
                    mset{},
                        mset{1},
                            mset{1, 1},
        ...
        using map = gtl::btree_map<int, int>;
        EXPECT_TRUE(gtl::VerifyTypeImplementsPhmapHashCorrectly({
                    map{},
                        map{{1, 0}},
                            map{{1, 1}},
        ...
        using mmap = gtl::btree_multimap<int, int>;
        EXPECT_TRUE(gtl::VerifyTypeImplementsPhmapHashCorrectly({
                    mmap{},
                        mmap{{1, 0}},
                            mmap{{1, 1}},
    ...
    TEST(Btree, ComparableMap) {
        gtl::btree_map<int, int> s1 = {{1, 2}};
        gtl::btree_map<int, int> s2 = {{2, 3}};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableMultimap) {
        gtl::btree_multimap<int, int> s1 = {{1, 2}};
        gtl::btree_multimap<int, int> s2 = {{2, 3}};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableSetWithCustomComparator) {
        ...
        gtl::btree_set<int, std::greater<int>> s1 = {1, 2};
        gtl::btree_set<int, std::greater<int>> s2 = {2, 3};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ExtractAndInsertNodeHandleSet) {
        ...
        gtl::btree_set<int> other;
        gtl::btree_set<int>::insert_return_type res = other.insert(std::move(nh));
        EXPECT_THAT(other, ElementsAre(3));
        EXPECT_EQ(res.position, other.find(3));
        EXPECT_TRUE(res.inserted);
        EXPECT_TRUE(res.node.empty());

        gtl::btree_set<int> src2 = {3, 4};
    ...
    TEST(Btree, ExtractTracking) {
        ...
        TestExtractWithTrackingForMap<
            gtl::btree_map<CopyableMovableInstance, MovableOnlyInstance>>();
        TestExtractWithTrackingForMap<
    ...
    TEST(Btree, ExtractAndInsertNodeHandleMultiSet) {
        ...
        gtl::btree_multiset<int> other;
        auto res = other.insert(std::move(nh));
        EXPECT_THAT(other, ElementsAre(3));
        EXPECT_EQ(res, other.find(3));

        gtl::btree_multiset<int> src2 = {3, 4};
        nh = src2.extract(src2.find(3));
        EXPECT_THAT(src2, ElementsAre(4));
        res = other.insert(std::move(nh));
    ...
    TEST(Btree, ExtractAndInsertNodeHandleMap) {
        gtl::btree_map<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
        auto nh = src1.extract(src1.find(3));
        EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
        gtl::btree_map<int, int> other;
        gtl::btree_map<int, int>::insert_return_type res =
        ...
        gtl::btree_map<int, int> src2 = {{3, 6}};
        nh = src2.extract(src2.find(3));
    ...
    TEST(Btree, ExtractAndInsertNodeHandleMultiMap) {
        gtl::btree_multimap<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
        auto nh = src1.extract(src1.find(3));
        EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
        gtl::btree_multimap<int, int> other;
        auto res = other.insert(std::move(nh));
        EXPECT_THAT(other, ElementsAre(Pair(3, 4)));
        EXPECT_EQ(res, other.find(3));

        gtl::btree_multimap<int, int> src2 = {{3, 6}};
        nh = src2.extract(src2.find(3));
        EXPECT_TRUE(src2.empty());
        res = other.insert(std::move(nh));
    ...
    TEST(Btree, InsertHintNodeHandle) {
        ...
        gtl::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> src =
            {{1, 2}, {3, 4}, {3, 5}};
        auto nh = src.extract(src.lower_bound(3));
        EXPECT_EQ(nh.value(), (InsertMultiHintData{3, 4}));
        gtl::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare>
            other = {{3, 1}, {3, 2}, {3, 3}};
    ...
    TEST(Btree, MergeIntoMultiMapsWithDifferentComparators) {
        gtl::btree_map<int, int, IntCompareToCmp> src1 = {{1, 1}, {2, 2}, {3, 3}};
        gtl::btree_multimap<int, int, std::greater<int>> src2 = {
            {5, 5}, {4, 1}, {4, 4}, {3, 2}};
        gtl::btree_multimap<int, int> dst;
    ...
    TEST(Btree, UserProvidedKeyCompareToComparators) {
        ...
        gtl::btree_set<int, KeyCompareToStrongOrdering> strong_set = {1, 2, 3};
    ...
    TEST(Btree, TryEmplaceWithHintWorks) {
        ...
        gtl::btree_map<int, int, Cmp> m(cmp);
        for (int i = 0; i < 128; ++i) {
            m.emplace(i, i);
        }
    ...
    TEST(Btree, TryEmplaceWithBadHint) {
        gtl::btree_map<int, int> m = {{1, 1}, {9, 9}};
    ...
    TEST(Btree, MoveAssignmentAllocatorPropagation) {
        ...
        {
            gtl::btree_set<MovableOnlyInstance, std::less<MovableOnlyInstance>,
