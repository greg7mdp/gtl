...
void BtreeTest()
{
    ...
    const std::vector<V> random_values =
        GenerateValuesWithSeed<V>(test_values, 4 * test_values, testing::GTEST_FLAG(random_seed));
...
TEST(Btree, NoHeterogeneousLookupWithoutAlias)
{
    ...
    using StringMultiSet = gtl::btree_multiset<std::string, NonTransparentCompare>;
    StringMultiSet ms;
    ms.insert("hello");
    ms.insert("world");
    ms.insert("world");
...
TEST(Btree, KeyCompareToAdapter)
{
    ...
    AssertKeyCompareToAdapted<std::greater<std::string_view>, std::string_view>();
...
TEST(Btree, ValueComp)
{
    ...
    EXPECT_TRUE(Identity(m2.value_comp()(std::make_pair("a", 0), std::make_pair("b", 0)) < 0));
    EXPECT_TRUE(Identity(m2.value_comp()(std::make_pair("b", 0), std::make_pair("b", 0)) == 0));
...
TEST(Btree, ExtractAndInsertNodeHandleMap)
{
    ...
    gtl::btree_map<int, int>::insert_return_type res = other.insert(std::move(nh));
    EXPECT_THAT(other, ElementsAre(Pair(3, 4)));
    EXPECT_EQ(res.position, other.find(3));
    EXPECT_TRUE(res.inserted);
    EXPECT_TRUE(res.node.empty());

    gtl::btree_map<int, int> src2 = {
        {3, 6}
    };
    nh = src2.extract(src2.find(3));
...
TEST(Btree, ExtractAndInsertNodeHandleMultiMap)
{
    ...
    gtl::btree_multimap<int, int> src2 = {
        {3, 6}
    };
    nh = src2.extract(src2.find(3));
    EXPECT_TRUE(src2.empty());
...
TEST(Btree, InsertHintNodeHandle)
{
    ...
    gtl::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> other = {
        {3,  1},
        { 3, 2},
        { 3, 3}
    };
    ...
    EXPECT_THAT(other,
                ElementsAre(InsertMultiHintData{ 3, 1 },
                            InsertMultiHintData{ 3, 2 },
                            InsertMultiHintData{ 3, 4 },
    ...
    EXPECT_THAT(other,
                ElementsAre(InsertMultiHintData{ 3, 5 },
                            InsertMultiHintData{ 3, 1 },
                            InsertMultiHintData{ 3, 2 },
                            InsertMultiHintData{ 3, 4 },
                            InsertMultiHintData{ 3, 3 }));
...
struct IntCompareToCmp
{
    gtl::weak_ordering operator()(const int& a, const int& b) const
    {
        ...
        if (a > b)
            return gtl::weak_ordering::greater;
...
TEST(Btree, TryEmplaceBasicTest)
{
    ...
    EXPECT_THAT(m,
                ElementsAreArray(std::vector<std::pair<int, std::string>>{
                    {1,   "one"},
                    { 2,  "two"},
                    { 42, "aaa"}
    }));
...
TEST(Btree, TryEmplaceWithBadHint)
{
    ...
    EXPECT_EQ(it, ++m.begin());
    EXPECT_THAT(m,
                ElementsAreArray(std::vector<std::pair<int, int>>{
                    {1,  1},
                    { 2, 2},
                    { 9, 9}
    }));
